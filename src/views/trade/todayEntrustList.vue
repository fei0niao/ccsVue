        ...
        <Row :gutter="10">
            <Card>
                <Row v-if="!this.contract_id">
                    ...
                    <timeRange :vm="vm" dateName='stock_finance_entrust_time' holderText="委托时间筛选"></timeRange>
                    </Col>
...
<script>
    ...
    import timeRange from '../common/timeRange.vue';
    export default {
        name: 'searchable-table',
        components: {excel, timeRange},
        props: ['contract_id'],
        data () {
            return {
                vm: this,
        ...
        methods: {
            ...
            getList (params) {
                return this.util.ajax(this).post('v1/todayEntrustList', params).then(res => {
                    res.data.list.forEach(function (item, index, input) {
                        item.new_cust_info = item.cust ? item.cust.new_cust_info : '';
                        item.new_bargain_average_price = item.today_parent_entrust ? item.today_parent_entrust.bargain_average_price : '';
                        item.new_bargain_amount = item.today_parent_entrust ? item.today_parent_entrust.bargain_amount : '';
                        item.new_bargain_price = item.today_parent_entrust ? item.today_parent_entrust.bargain_price : '';
                    });
                    return res.data;
                });
            },
            ...
            create () {
                this.$refs.create.validate((valid) => {
                    ...
                    this.util.ajax(this).post('v1/todayEntrust/create', this.createForm).then(res => {
            ...
            getInfo ($id, postParams, modalName) {
                this.util.ajax(this).post('v1/todayEntrustInfo/' + $id, postParams).then(res => {
                    this.info = res.data;
                    this[modalName] = true;
                });
            },
            ...
            update ($id, postParams, modalName) {
                this.util.ajax(this).post('v1/todayEntrust/update/' + $id, postParams).then(res => {
                    if (modalName) {
