...
<template>
    <div>
                ...
                <Row slot="title">
                    <ButtonGroup>
                        <Button @click="highLight!=1?highLight=1:''"
                                :type="highLight == 1?'primary':null">今日系统可分配买入
                        </Button>
                        <Button @click="highLight!=2?highLight=2:''"
                                :type="highLight == 2?'primary':null">历史系统可分配买入
                        </Button>
                    </ButtonGroup>
                </Row>
                <Row>
        ...
        <template>
            <Modal
                v-model="assignBuyModal"
                title="系统补买"
                okText="确定"
                :loading="okloading"
                @on-ok="assignBuy">
                <Form ref="assignBuy" :rules="assignBuyRuler" :model="info" label-position="right" :label-width="120">
                    <FormItem label="分配提示">
                        成交股票 : {{info.stock_code + ' ' + info.stock_name}}
                        <br/>
                        成交情况 : {{'买入' + info.bargain_amount + '股, 花费' + info.bargain_price + '元'}}
                    </FormItem>
                    <FormItem label="合约委托ID" prop="stock_finance_entrust_id">
                        <Input v-model="info.stock_finance_entrust_id"></Input>
                    </FormItem>
                </Form>
            </Modal>
        </template>
    ...
    export default {
        ...
        data () {
            return {
                ...
                okloading: true,
                loading: false,
                assignBuyModal: false,
                assignBuyRuler: {
                    stock_finance_entrust_id: [
                        { required: true, message: '合约委托ID不能为空', trigger: 'blur' }
                    ]
                },
                info: {},
                highLight: 1,
                                ...
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            ...
                                            this.getInfo(params.row.id, postParams, 'assignBuyModal');
                                        }
                                    }
                                }, '系统补买')
                            ]);
        ...
        watch: {
            highLight: function () {
                if (this.highLight == 1) {
                    this.highLightInfo = {url: 'todayParentEntrust', name: '今日系统可分配买入', javaUrl: 'assignBuyDayDeal'};
                } else {
                    this.highLightInfo = {url: 'historyParentEntrust', name: '历史系统可分配买入', javaUrl: 'assignBuyHistoryDeal'};
                }
                this.init();
            }
        },
        methods: {
            ...
            assignBuy () {
                this.util.valid(this, 'assignBuy').then(res => {
                    if (!res) return;
                    let postParams = {
                        id: this.info.id,
                        stockFinanceEntrustId: this.info.stock_finance_entrust_id
                    };
                    this.util.ajax(this).post('v1/requestApi/' + this.highLightInfo.javaUrl, postParams).then(res => {
                        this.assignBuyModal = false;
                        this.util.notice(this, res);
                        this.init();
                    });
                });
